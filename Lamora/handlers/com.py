from telethon import events from database import models as db from utils.helpers import admin_only, extract_user_id, extract_text

def register(client): @client.on(events.NewMessage(pattern="/start")) async def start(event): await db.init_db() await event.reply("Bot aktif dan siap digunakan.")
